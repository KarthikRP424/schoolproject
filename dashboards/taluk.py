...
dashboards/taluk.py
-------------------
Defines the dashboard view for Taluk Education Officers (TEO).
Limits scope to the officer's assigned taluk and district.
"""
...
from components.timelines import draw_issue_status_timeline, draw_audit_trail_timeline
from services.audit_service import get_audit_trail

def render_taluk_dashboard(user_profile: dict):
    """
    Renders analytics, maps, issues, and inspection forms for Taluk Officers.
    """
    district = user_profile.get("district")
    taluk = user_profile.get("taluk")
    st.title(f"🏛️ Taluk Education Office: {taluk} ({district} District)")
    st.write(f"Welcome, Taluk Officer. Displaying monitoring data for **{taluk} Taluk** schools.")

    # Load notifications for this role
    notifications = get_unread_notifications(user_profile)
    if notifications:
        with st.expander(f"🔔 Local Alerts ({len(notifications)})", expanded=True):
    ...
    # 1. Fetch Taluk Schools
    taluk_schools = execute_query("SELECT * FROM schools WHERE district = ? AND taluk = ?;", (district, taluk))
    
    # 2. Fetch Taluk Issues
    filters = {"district": district, "taluk": taluk}
    issues = get_filtered_issues(user_profile, filters)

    # ─────────────────────────────────────────────────────────────────────────
    # TALUK OVERVIEW METRICS
    # ─────────────────────────────────────────────────────────────────────────
    st.subheader("📊 Taluk KPIs")
    total_sch = len(taluk_schools)
    ...
    with m1:
        st.metric("Schools in Taluk", total_sch)
    with m2:
        st.metric("Active Complaints", open_iss)
    with m3:
        st.metric("🔴 Urgent Situations", urgent_iss)
    with m4:
        avg_h = sum(s["health_score"] for s in taluk_schools) / max(total_sch, 1)
        st.metric("Average Health Index", f"{avg_h:.1f}/100")

    # Display Map
    st.subheader("🗺️ Taluk Schools Risk Map")
    draw_schools_map(taluk_schools)

    tab_issues, tab_inspect = st.tabs(["📋 Active Complaints Register", "🔍 Schedule & Log Inspections"])

    # ═════════════════════════════════════════════════════════════════════════
    # TAB 1: LOGS & ASSIGNMENTS
    # ═════════════════════════════════════════════════════════════════════════
    with tab_issues:
        st.subheader("📋 Active Issues Register")
        if not issues:
            st.info("No active open reports listed in your taluk.")
        else:
            df_issues = pd.DataFrame(issues)
            
            # Filter bar
            f_priority, f_status = st.columns(2)
            with f_priority:
            ...
            if filtered_df.empty:
                ...
                display_cols = ["report_id", "school_name", "category", "priority_level", "status", "submitted_time"]
                st.dataframe(filtered_df[display_cols].rename(columns={
                    "report_id": "ID", "school_name": "School Name",
    ...
    with tab_inspect:
        ...
        with st.form("taluk_inspection_form"):
            sch_select = st.selectbox(
                "Target School Site:",
                options=[s["id"] for s in taluk_schools],
                format_func=lambda sid: next(s["name"] for s in taluk_schools if s["id"] == sid)
            )
